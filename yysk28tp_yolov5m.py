...
class Yolov5:
    ...
    def resize_image(self, srcimg, keep_ratio=True):
        ...
        if keep_ratio and srcimg.shape[0] != srcimg.shape[1]:
            ...
            if hw_scale > 1:
                ...
                img = cv2.copyMakeBorder(img, 0, 0, left, self.input_shape[1] - neww - left, cv2.BORDER_CONSTANT, value=0)  # add border
            else:
    ...
    def postprocess(self, frame, outs, pad_hw):
        ...
        for i in indices:
            ...
            width = box[2]
            height = box[3]
            result.append([left, top, width, height, classIds[i], confidences[i]])
    ...
    def drawPred(self, frame, classId, conf, left, top, right, bottom):
        ...
        cv2.putText(frame, label, (left, top - 10), cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 255, 0), thickness=1)
        return frame

    def detect(self, srcimg):
        img, newh, neww, top, left = self.resize_image(srcimg)
        #print(img.shape,newh,neww,top,left)
        ...
        #print(outs.shape)
        #print(outs)
        out_img, res = self.postprocess(srcimg, outs, (newh, neww, top, left))
        infer_time = 'Inference Time: ' + str(int(cost_time * 1000)) + 'ms'
        cv2.putText(out_img, infer_time, (20, 50), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (255, 255, 255), thickness=1)
        return out_img, res
...
if __name__ == '__main__':
    ...
    net = Yolov5(args.modelpath, args.classfile, confThreshold=args.confThreshold, nmsThreshold=args.nmsThreshold)
    srcimg, res = net.detect(srcimg)
